...
import { E2ETestHarness, type E2EScenario } from "./E2ETestHarness.js";
...
async function main() {
  ...
  const specificScenario =
    scenarioIndex !== -1 ? parseInt(args[scenarioIndex + 1], 10) : null;
  ...
  if (!config.testRepo) {
    ...
    console.error(
      "   Example: export TEST_REPO=miyabi-e2e-test (repository name only, not full URL)"
    );
    process.exit(1);
  }
  ...
  if (!config.testOwner) {
    ...
    console.error(
      "   Set it with: export TEST_OWNER=your-github-username-or-org"
    );
  ...
  try {
    if (specificScenario !== null) {
      ...
      const scenario = scenarios.find((s: E2EScenario) => s.id === specificScenario);

      if (!scenario) {
        console.error(
          `❌ ERROR: Scenario ${specificScenario} not found. Available scenarios: ${scenarios.map((s: E2EScenario) => s.id).join(", ")}`
        );
        process.exit(1);
      }

      console.log(
        `Running single scenario: ${scenario.id} - ${scenario.name}\n`
      );
