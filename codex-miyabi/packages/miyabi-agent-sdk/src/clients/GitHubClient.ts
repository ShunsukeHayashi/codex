...
export class GitHubClient {
  ...
  async getIssue(
    ...
    try {
      ...
      console.log(
        `[GitHubClient] Fetched issue #${issueNumber}: ${data.title}`
      );
      ...
      console.error(
        `[GitHubClient] Failed to fetch issue #${issueNumber}:`,
        error
      );
      throw new Error(
  ...
  async addLabels(
    ...
    try {
      ...
      console.log(
        `[GitHubClient] Added labels to #${issueNumber}:`,
        labels
      );
    } catch (error) {
      console.error(
        `[GitHubClient] Failed to add labels to #${issueNumber}:`,
        error
      );
      throw new Error(
  ...
  async getFileContent(
    ...
    try {
      ...
      if (Array.isArray(data) || data.type !== "file") {
        console.warn(
          `[GitHubClient] Path is not a file: ${path}`
        );
        return null;
      }
      ...
      console.error(
        `[GitHubClient] Failed to fetch file ${path}:`,
        error
      );
      return null;
    }
  ...
  async createBranch(
    ...
    try {
      ...
      console.log(
        `[GitHubClient] Created branch: ${branchName} from ${baseBranch}`
      );
    } catch (error) {
      console.error(
        `[GitHubClient] Failed to create branch ${branchName}:`,
        error
      );
      throw new Error(
  ...
  async commitFiles(params: {
    ...
    try {
      ...
      console.log(
        `[GitHubClient] Committed ${params.files.length} files to ${params.branch}: ${commit.sha}`
      );

      return commit.sha;
    } catch (error) {
      console.error(
        `[GitHubClient] Failed to commit files to ${params.branch}:`,
        error
      );
      throw new Error(
  ...
  async createPullRequest(params: {
    ...
    try {
      ...
      console.log(
        `[GitHubClient] Created ${params.draft ? "Draft " : ""}PR #${data.number}: ${data.html_url}`
      );
  ...
  async addComment(
    ...
    try {
      ...
      console.log(
        `[GitHubClient] Added comment to #${issueNumber}`
      );
    } catch (error) {
      console.error(
        `[GitHubClient] Failed to add comment to #${issueNumber}:`,
        error
      );
      throw new Error(
  ...
  async checkRateLimit(): Promise<{
    ...
    try {
      ...
      if (result.remaining < 100) {
        console.warn(
          `[GitHubClient] Rate limit low: ${result.remaining}/${result.limit} (resets at ${result.reset.toISOString()})`
        );
      }
  ...
  async waitIfNeeded(): Promise<void> {
    ...
    if (remaining < 100) {
      ...
      if (waitMs > 0) {
        console.warn(
          `[GitHubClient] Waiting ${Math.ceil(waitMs / 1000)}s for rate limit reset...`
        );
