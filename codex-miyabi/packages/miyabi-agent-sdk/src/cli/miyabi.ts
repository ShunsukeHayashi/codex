...
async function runAnalyze(
  ...
  if (!config.owner || !config.repo || !config.githubToken) {
    console.error(
      "❌ Error: --repo and --github-token are required for analyze command"
    );
    process.exit(1);
  }
...
async function runGenerate(
  ...
  if (!config.owner || !config.repo || !config.githubToken) {
    console.error(
      "❌ Error: --repo and --github-token are required for generate command"
    );
    process.exit(1);
  }
 ...
 * Run review command
 */
async function runReview(files: string[], config: CLIConfig): Promise<void> {
  if (files.length === 0) {
...
async function runWorkflow(
  ...
  if (!config.owner || !config.repo || !config.githubToken) {
    console.error(
      "❌ Error: --repo and --github-token are required for workflow command"
    );
    process.exit(1);
  }
